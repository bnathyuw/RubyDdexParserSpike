...
class DdexReader
	...
	def has_deal?(release_reference, territory_code, date)
		query = "//ReleaseDeal[DealReleaseReference/.=$release_reference]/Deal/DealTerms[TerritoryCode/.=$territory_code and translate(ValidityPeriod/StartDate/., '-', '') <= translate($date, '-', '')]"
		variables = {
			"release_reference" => release_reference,
			"territory_code" => territory_code,
			"date" => date.to_s
